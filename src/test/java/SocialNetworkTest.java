...
public class SocialNetworkTest {
    ...
    public void averageNumberOfMessagesByDayOfWeek() {
        ...
        Document group2 = Document.parse("{\n" + "    \"$group\": {\n" + "      \"_id\": \"$date_of_birth\",\n" + "  " +
                "    \"average\": {\n" + "        \"$avg\": \"$value\"\n" + "      }\n" + "    }\n" + "  }");
        user.aggregate(Arrays.asList(addFields1, group1, addFields2, group2, project)).into(results);
    ...
    public void minNumberOfWatchedMoviesByUsersWithMoreThan100Friends() {
        ...
        Document lookup = Document.parse(" { $lookup:\n        {\n           from: \"user\",\n           localField: "
                ...
                + "  " + "}");
        Document match = Document.parse("{\"$match\":{\"movies.is_watched\":{\"$eq\":true}}}");
        Document group_count_user_id = Document.parse(" {\n" + "      " +
                " $group:\n" + "         {\n" + "        " +
                "   _id: \"$user_id\",\n" + "       " +
                "    count: { $count: { } }\n" + "       " +
                "  }\n" + "     }");
        Document group_min_number = Document.parse(" {\n" + "      " +
                " $group:\n" + "         {\n" + "        " +
                "   _id: \"$_id\",\n" + "           " +
                "minNumber: { $min: \"$count\" }" + "  " +
                "       }\n" + "     }");
    ...
    private void insertData() {
        ...
        userDocuments = Arrays.asList(
                new Document("user_id", 1)
                        ...
                        .append("city", "Krakow")
                        .append("value", 4)
                        .append("movies", Arrays.asList(
                                new Document("movie_id", 1)
                                        .append("title", "AAAAA")
                        ...
                        .append("audio_tracks", Arrays.asList(
                                new Document("audio_track_id", 1)
                                        .append("title", "CCCCC")
        ...
        friendshipDocuments = Arrays.asList(
                new Document("friendship_id", 1)
                        .append("user_sender_id", 1)
                        .append("user_receiver_id", 2)
                        .append("value", 18)
                        .append("creation_date", df.parse("2022-04-15")),
                new Document("friendship_id", 2)
                        .append("user_sender_id", 1)
                        .append("user_receiver_id", 3)
                        .append("value", 5)
                        .append("creation_date", df.parse("2022-05-21")));
